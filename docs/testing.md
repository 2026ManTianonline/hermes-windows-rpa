...
## 3. 端到端场景测试：WeChat 4.x 发送消息 (2026-06-06 新增)

### 3.1 测试目标

验证 **纯本地方案**（0 vision 模型调用）发送中文消息到微信群：

```
pygetwindow → 布局公式 → 搜索框搜群 → pyperclip 中文 → 发送
```
...
| 步骤 | 方案 | 耗时 | vision 调用 |
|------|------|------|:----------:|
| 窗口定位 | pygetwindow → (52,107) 752x697 | 30ms | **0** |
| UI 坐标 | 公式反推 → 搜索框/输入框/发送按钮 | 0ms | **0** |
| 联系人识别 | EasyOCR → 24信安2班冯智财 94% | 3-5s | **0** |
| 中文输入 | Windows pyperclip + Ctrl+V | 0.5s | **0** |
| **总计** | | **~5s** | **0** |
...
| 联系人 | 识别结果 | 置信度 |
|--------|---------|:------:|
| Mss(南职支撑组) | ✅ Mss (南职支撑组) | 75% |
| 24信安2班冯智财 | ✅ 24信安2班冯智财 | 94% |
| 8栋301 | ✅ 8栋301 | 77% |
| 梁昊泽13377192821 | ✅ 梁昊泽13377192821 | 93% |
| 网络安全技术协会. | ✅ 网络安全技术协会. | 99% |
| 微信支付 | ✅ 微信支付 | 89% |

**关键发现**：EasyOCR 中文输出通过 UTF-8 JSON 文件方案，中文完全正确显示（详见 `scripts/easyocr_reader.py`）。
